...
def add_random_reactions(message_id, count=10):
    try:
        # Prende le reazioni attuali
        chat_reactions = bot.get_message_reactions(CHANNEL_ID, message_id)
        
        # Lista di emoji attuali (se esistono già)
        existing_reactions = [reaction.emoji for reaction in chat_reactions] if chat_reactions else []

        # Seleziona emoji casuali, evitando duplicati
        new_reactions = random.sample([e for e in REACTION_EMOJIS if e not in existing_reactions], count)

        # Combina le reazioni attuali con quelle nuove
        all_reactions = existing_reactions + new_reactions

        # Invia tutte le reazioni
        bot.set_message_reaction(
            chat_id=CHANNEL_ID,
            message_id=message_id,
            reaction=[telebot.types.ReactionTypeEmoji(emoji=emoji) for emoji in all_reactions]
        )
        print(f"✅ Aggiunte {count} reazioni al post {message_id}")
