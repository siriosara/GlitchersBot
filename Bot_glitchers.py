...
def add_random_reactions(message_id, count=10):
    try:
        ...
        max_new_reactions = min(count, len(REACTION_EMOJIS) - len(existing_reactions))

        if max_new_reactions <= 0:
            print(f"⚠️ Nessuna nuova reazione disponibile per il post {message_id}")
            return
